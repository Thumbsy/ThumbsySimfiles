...
function player_mods__add_speed_mod_absolute(beat, tween_time, scroll_speed, pn)
	...
	if pn then
		if SCREENMAN:GetTopScreen():GetChild('PlayerP' .. pn) then
			...
			if player_mods__plr_init_speed[pn][2] == 'x' then
			...
			elseif player_mods__plr_init_speed[pn][2] == 'C' then
			...
			elseif player_mods__plr_init_speed[pn][2] == 'm' then
				scroll_diff = math.abs(scroll_speed - prev_player_speed[pn])
		...
		for i,v in pairs(player_mods__plr_init_speed) do		
			if v[2] == 'x' then
				...
				user_content__mods[#user_content__mods+1] = {beat, '*' .. base_rate * xrate_diff .. ' ' .. xrate .. 'x', v[1]}
				prev_player_speed[i] = xrate
			elseif v[2] == 'C' then
				scroll_diff = math.abs(scroll_speed - prev_player_speed[i])
				user_content__mods[#user_content__mods+1] = {beat, '*'.. (base_rate * scroll_diff) / user_content__bpm .. ' ' .. 'C' .. scroll_speed, v[1]}
				prev_player_speed[i] = scroll_speed
			elseif v[2] == 'm' then
				scroll_diff = math.abs(scroll_speed - prev_player_speed[i])
				user_content__mods[#user_content__mods+1] = {beat, '*' .. base_rate * scroll_diff .. ' ' .. 'm' .. scroll_speed, v[1]}
				prev_player_speed[i] = scroll_speed
			else
				-- We shouldn't be ending up here, if we do, something went wrong.
...
function player_mods__add_speed_mod_relative(beat, tween_time, multiplier, pn)
	...
	if pn then
		if SCREENMAN:GetTopScreen():GetChild('PlayerP' .. pn) then
			speed = multiplier * player_mods__plr_init_speed[pn][3]
		...
		for i,v in pairs(player_mods__plr_init_speed) do
			...
			if v[2] == 'x' then
				user_content__mods[#user_content__mods+1] = {beat, '*' .. base_rate * speed_diff .. ' ' .. speed .. 'x', v[1]}
				prev_player_speed[i] = speed
			elseif v[2] == 'C' then
				user_content__mods[#user_content__mods+1] = {beat, '*' .. (base_rate * speed_diff) / user_content__bpm .. ' ' .. 'C' .. speed, v[1]}
				prev_player_speed[i] = speed
			elseif v[2] == 'm' then
				user_content__mods[#user_content__mods+1] = {beat, '*' .. base_rate * speed_diff .. ' ' .. 'm' .. speed, v[1]}
				prev_player_speed[i] = speed
			else
				-- We shouldn't be ending up here, if we do, something went wrong.
				SCREENMAN:SystemMessage('[ERROR] No valid speed type detected for Player ' .. v[1])
...
function player_mods__linear_breathing(beat, undo_after, tween_time, multiplier, pn)
	...
	if pn then
		...
		for i,v in pairs(player_mods__plr_init_speed) do
			...
			if v[2] == 'x' then
				user_content__mods[#user_content__mods+1] = {beat, '*' .. base_rate * speed_diff .. ' ' .. speed .. 'x', v[1]}
				user_content__mods[#user_content__mods+1] = {beat+undo_after, '*' .. base_rate * speed_diff .. ' ' .. prev_player_speed[i] .. 'x', v[1]}
			elseif v[2] == 'C' then
				user_content__mods[#user_content__mods+1] = {beat, '*' .. (base_rate * speed_diff) / user_content__bpm .. ' ' .. 'C' .. speed, v[1]}
				user_content__mods[#user_content__mods+1] = {beat+undo_after, '*' .. (base_rate * speed_diff) / user_content__bpm .. ' ' .. 'C' .. prev_player_speed[i], v[1]}
			elseif v[2] == 'm' then
				user_content__mods[#user_content__mods+1] = {beat, '*' .. base_rate * speed_diff .. ' ' .. 'm' .. speed, v[1]}
				user_content__mods[#user_content__mods+1] = {beat+undo_after, '*' .. base_rate * speed_diff .. ' ' .. 'm' .. prev_player_speed[i], v[1]}
			else
				-- We shouldn't be ending up here, if we do, something went wrong.
				SCREENMAN:SystemMessage('[ERROR] No valid speed type detected for Player ' .. v[1])
			end
...
function player_mods__inject_speed_mods(enabled_players, plr_init_speed)
	...
	for k in pairs(enabled_players) do
		prev_player_speed[k] = plr_init_speed[k][3]
	end
